...
b_countries = list(filter(lambda item: item[0]=='B', countries))
print(b_countries)
...
first_names = [a2 for (a1,a2) in people ]
print(first_names)
...
passed = [a1 for (a1,a2) in students if a2 >= 70]
...
p = zip(l1,l2)
opposites = list(filter(lambda x : len(x[0])>3 and len(x[1])>3, p))
print(opposites)
...
endangered = [a1 for (a1,a2) in pop_info if a2 < 2500]
print(endangered)
